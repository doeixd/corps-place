...
const denormalize = (value, mean, std) => value * (std > 1e-6 ? std : 1) + mean;
...
class MaskedSoftmax extends tf.layers.Layer {
    ...
    call(inputs, kwargs) {
        return tf.tidy(() => {
            const [scoresRaw, maskRaw] = inputs;
...
class LastStepLayer extends tf.layers.Layer {
    ...
    call(inputs) {
        return tf.tidy(() => {
            ...
            return seq.slice([0, SEQ_LEN - 1, 0], [-1, 1, -1]).squeeze([1]);
        });
    }
    getConfig() { return { ...super.getConfig() }; }
}
...
class CategoryLayer extends tf.layers.Layer {
    ...
    constructor(config) {
        ...
        this.catA = tf.tensor2d(MatrixM.map((row, i) => row.map((val, j) => val * stats.recapStd[j] / (stats.categoryStd[i] > 1e-6 ? stats.categoryStd[i] : 1)))).transpose();
    ...
    call(inputs) {
        const recap = Array.isArray(inputs) ? inputs[0] : inputs;
        return tf.tidy(() => {
...
class TotalLayer extends tf.layers.Layer {
    ...
    call(inputs) {
        const cat = Array.isArray(inputs) ? inputs[0] : inputs;
        return tf.tidy(() => {
...
const JUDGE_INDEX_MAP = JSON.parse(fs.readFileSync(JUDGE_INDEX_PATH, "utf-8"));
const JUDGE_COUNT = Object.keys(JUDGE_INDEX_MAP).length;
const CORPS_INDEX_MAP = JSON.parse(fs.readFileSync(CORPS_INDEX_PATH, "utf-8"));
...
const coverageWeight = (epoch, start, ramp, maxW) => {
    if (epoch < start)
        return 0;
...
class SequenceDataProviderV9 {
    ...
    openShows;
    allShows;
    constructor(rows, epoch, batchSize = BATCH_SIZE) {
        this.rows = rows;
        this.epoch = epoch;
    ...
    sampleShows(shows, targetCount, seed) {
        if (shows.length === 0)
            return [];
...
function buildDataRows(rows) {
    ...
    for (const showKey in shows) {
        ...
        for (const { row, recap, total } of parsedShow) {
            const rawSeq = JSON.parse(row.x_sequence_json);
            ...
            const judgeIndices = JSON.parse(row.judge_indices_json);
            // const resids = JSON.parse(row.y_residuals_json) as Record<string, number>; // Unused
            if (seq.length !== SEQ_LEN || (seq[0] && seq[0].length !== FEAT_DIM))
                continue;
            if (stat.length !== RAW_STATIC_DIM)
                continue;
            ...
            const contextFeatures = []; // Empty array - no leaky features
...
function applyBaselines(rows, historyRows) {
    const historySet = new Set(historyRows);
    const corpsMap = {};
    for (const row of rows) {
        if (!corpsMap[row.corpsId])
            corpsMap[row.corpsId] = [];
    ...
    for (const corpsId in corpsMap) {
        ...
        const recapHistory = Array.from({ length: CAPTION_COUNT }, () => []);
        for (const row of corpsRows) {
            ...
            for (let i = 0; i < CAPTION_COUNT; i++) {
                ...
                if (val !== undefined && val !== null) {
                    ...
                    if (recapHistory[i].length > 3)
                        recapHistory[i].shift();
                }
            }
        }
    }
...
function computeTargetStats(rows) {
    const deltaSeries = Array.from({ length: CAPTION_COUNT }, () => []);
    const recapSeries = Array.from({ length: CAPTION_COUNT }, () => []);
    const categorySeries = Array.from({ length: 3 }, () => []); // GE, Visual, Music
    const totalSeries = [];
    for (const row of rows) {
...
function normalizeValue(value, meanValue, stdValue) {
    if (!Number.isFinite(stdValue) || stdValue < 1e-6)
        return 0;
    return (value - meanValue) / stdValue;
}
function getRecapFromStep(step, captionIndex) {
...
function computeBaselineMae(samples, stats) {
    let zeroSum = 0;
    let meanSum = 0;
    let emaSum = 0;
    let count = 0;
...
function buildSamples(rows, stats, seqLen, identityDropoutRate, seed, epoch = 0, baselineDropoutRate = BASELINE_DROPOUT_RATE, baselineNoiseStd = BASELINE_NOISE_STD_PTS) {
    const samples = [];
    ...
    for (const row of rows) {
        ...
        const baselineNormVector = []; // Normalized model input [8]
...
function createDataset(samples, batchSize, shuffle, seed) {
...
let sequenceBuffer = null;
let staticBuffer = null;
let maskBuffer = null;
let judgeIdsBuffer = null;
let corpsIdBuffer = null;
let baselineBuffer = null;
let historyLenBuffer = null;
let showIdBuffer = null;
let ysBuffer = null;
let currentBufferSize = 0;
// PERFORMANCE: Batched tensor creation - creates tensors once per batch instead of per sample
function buildBatchTensors(batchSamples, scales) {
    ...
    for (let i = 0; i < batchSize; i++) {
        ...
        // Assertions
        if (sample.xs[1].length !== TOTAL_STATIC_DIM)
            throw new Error(`Invalid static feature dim at index ${i}: got ${sample.xs[1].length}, expected ${TOTAL_STATIC_DIM}`);
        if (sample.xs[3].length !== CAPTION_COUNT)
            throw new Error(`Invalid judge_ids length at index ${i}`);
        if (sample.xs[5].length !== CAPTION_COUNT)
            throw new Error(`Invalid baseline_recap length at index ${i}`);
        ...
        if (rawCorpsId < 0 || rawCorpsId >= CORPS_COUNT)
            throw new Error(`Corps ID ${rawCorpsId} out of range (max ${CORPS_COUNT - 1})`);
...
function* batchGenerator(samples, batchSize, shuffle, seed, scales) {
    const rng = seededRandom(seed);
    const showMap = new Map();
    for (const sample of samples) {
...
function createLoss(stats, widthFloorPts, widthFloorWeight, rankingWeight) {
    ...
    const lossFn = (yTrue, yPred, weights, historyLen, showIds, returnComponents = false, regLoss) => tf.tidy(() => {
        ...
        const weightedCaptionMean = (lossByCap, weights) => {
        ...
        const weightedQ10 = weightedCaptionMean(q10Loss, deltaWeightTensor);
        const weightedQ50 = weightedCaptionMean(q50Loss, deltaWeightTensor);
        const weightedQ90 = weightedCaptionMean(q90Loss, deltaWeightTensor);
        const deltaLoss = tf.add(tf.mul(tf.scalar(weights.deltaWeight), weightedQ50), tf.mul(tf.scalar(weights.quantileWeight), tf.add(weightedQ10, weightedQ90)));
        const recapError = tf.sub(recapTrue, recapPred);
        const recapSq = tf.square(recapError);
        ...
        const rankingLoss = (() => {
            ...
            const target = tf.add(tf.cast(gtMask, "float32"), tf.mul(tf.cast(zeroMask, "float32"), tf.scalar(0.5)));
            const predProb = tf.clipByValue(tf.sigmoid(diffPred), 1e-7, 1 - 1e-7);
            const lossMatrix = tf.neg(tf.add(tf.mul(target, tf.log(predProb)), tf.mul(tf.sub(tf.scalar(1), target), tf.log(tf.sub(tf.scalar(1), predProb)))));
...
async function main() {
    ...
    const seedrandom = tf.util.seedrandom;
    if (seedrandom) {
  ...
  `);
    ...
    if (initialValSamples.length) {
        console.log(`Baselines(monitoring forecast MAE): zero = ${baselines.baselineZero.toFixed(4)}, ` +
            `mean = ${baselines.baselineMean.toFixed(4)}, ema = ${baselines.baselineEma.toFixed(4)} `);
    }
    const { lossFn, dispose: disposeLoss } = createLoss(stats, args.widthFloorPts, args.widthFloorWeight, args.rankingWeight);
    ...
    const saveModel = async (modelToSave, dir) => {
            ...
            save: async (modelArtifacts) => {
                if (modelArtifacts.weightData) {
                    ...
                    if (modelArtifacts.weightData instanceof ArrayBuffer) {
                        ...
                        buffer = Buffer.concat(modelArtifacts.weightData.map((chunk) => Buffer.from(new Uint8Array(chunk))));
                    }
    ...
    const judgeFlat = tf.layers.flatten().apply(judgeEmbedding);
    // Corps Embedding
    ...
    const corpsFlat = tf.layers.flatten().apply(corpsEmbedding);
    ...
    const lstm1 = tf.layers
        ...
        .apply(seqInput);
    const norm1 = tf.layers.layerNormalization().apply(lstm1);
    const lstm2 = tf.layers
        ...
        .apply(norm1);
    const norm2 = tf.layers.layerNormalization().apply(lstm2);
    let attentionInput = norm2;
    if (args.useMha) {
        try {
            const require = createRequire(import.meta.url);
            const { MultiHeadAttention } = require("@tensorflow/tfjs-layers/dist/layers/nlp/multihead_attention");
            const selfAttended = new MultiHeadAttention({ numHeads: 4, keyDim: 16 }).apply(norm2, {
                value: norm2,
                key: norm2,
            });
            const attentionResidual = tf.layers.add().apply([norm2, selfAttended]);
            attentionInput = tf.layers.layerNormalization().apply(attentionResidual);
            attentionInput = tf.layers.dropout({ rate: 0.1 }).apply(attentionInput);
        }
        catch (error) {
            console.warn("MultiHeadAttention failed; falling back to norm2.", error);
        }
    }
    const attentionScores = tf.layers.dense({ units: 1, activation: "tanh" }).apply(attentionInput);
    const attentionScoresFlat = tf.layers.reshape({ targetShape: [SEQ_LEN] }).apply(attentionScores);
    const maskInput = tf.input({ shape: [SEQ_LEN], name: "mask" });
    const attentionWeightsFlat = new MaskedSoftmax().apply([attentionScoresFlat, maskInput]);
    const attentionWeights = tf.layers
        .reshape({ targetShape: [SEQ_LEN, 1] })
        .apply(attentionWeightsFlat);
    ...
    const contextFlat = new AttentionPoolingLayer({ name: "attention_pool" }).apply([attentionWeights, attentionInput]);
    const lastStepFlat = new LastStepLayer({ name: "last_step" }).apply(seqInput);
    // Phase 1: Add baseline to inputs
    // Milestone 2: Removed corpsFlat from trunk concatenation (Residual Only)
    const concat = tf.layers.concatenate().apply([contextFlat, staticInput, judgeFlat, baselineInput, lastStepFlat]);
    const d1 = tf.layers
        .dense({
        units: 128,
        activation: "relu",
        ...
        .apply(concat);
    const d1Drop = tf.layers.dropout({ rate: args.dropoutDense1 }).apply(d1);
    const d2 = tf.layers
        .dense({
        units: 64,
        activation: "relu",
        ...
        .apply(d1Drop);
    const d2Drop = tf.layers.dropout({ rate: args.dropoutDense2 }).apply(d2);
    // Phase 2: Strength Head (from context)
    const strength = tf.layers.dense({ units: 16, activation: "relu", name: "strength" }).apply(contextFlat);
    const skipConcat = tf.layers.concatenate().apply([d2Drop, staticInput, strength]);
    // Phase 3: Judge Mixed Effects
    // Gate: judgeBias = Dense(judgeFlat) * judge_bias_scale
    const judgeBiasRaw = tf.layers.dense({ units: CAPTION_COUNT, name: "judge_bias_raw", kernelRegularizer: tf.regularizers.l2({ l2: 1e-4 }) }).apply(judgeFlat);
    const judgeBias = new LambdaScale({ name: "judge_bias_gated" }).apply([judgeBiasRaw, judgeBiasScaleInput]);
    ...
    const corpsCorr = new LambdaScale({ name: "corps_corr_gated" }).apply([corpsCorrRaw, corpsScaleInput]);
    // Summation: base + judge + corps
    const deltaQ50 = tf.layers.add({ name: "delta_q50" }).apply([deltaQ50Base, judgeBias, corpsCorr]);
    ...
    const widthConcat = tf.layers.concatenate({ name: "width_concat" }).apply([skipConcat, judgeFlat, historyLenInput]);
    const q10WidthRaw = q10WidthLayer.apply(widthConcat);
    const q90WidthRaw = q90WidthLayer.apply(widthConcat);
    const q10Width = tf.layers.activation({ activation: "softplus", name: "q10_width" }).apply(q10WidthRaw);
    const q90Width = tf.layers.activation({ activation: "softplus", name: "q90_width" }).apply(q90WidthRaw);
    // q10_delta = delta_q50 - q10_width
    const q10WidthNeg = new NegationLayer({ name: "q10_width_neg" }).apply(q10Width);
    const q10Delta = tf.layers.add({ name: "q10_delta" }).apply([deltaQ50, q10WidthNeg]);
    const q90Delta = tf.layers.add({ name: "q90_delta" }).apply([deltaQ50, q90Width]);
    // Derivation of absolute recap: recap_norm = baseline_norm + A * delta_q50 + C
    const recapHead = new RecapLayer({ name: "recap_head", stats }).apply([deltaQ50, baselineInput]);
    // Category Derivation
    const categoryHead = new CategoryLayer({ name: "category_head", stats }).apply(recapHead);
    // Total Derivation
    const totalHead = new TotalLayer({ name: "total_head", stats }).apply(categoryHead);
    const output = tf.layers
        .concatenate({ name: "output" })
        .apply([q10Delta, deltaQ50, q90Delta, recapHead, categoryHead, totalHead]);
        ...
        if (fs.existsSync(args.loadModel)) {
            ...
            if (fs.existsSync(weightsPath) && fs.existsSync(modelJsonPath)) {
                ...
                for (const spec of weightSpecs) {
                    const byteLength = spec.shape.reduce((a, b) => a * b, 1) * 4;
    ...
    tf.tidy(() => {
        ...
        const outputTensor = testOutputs;
    ...
    console.log(`Hyperparameters: lstm1=${args.lstm1Units}, lstm2=${args.lstm2Units}, dropout=${args.dropoutLstm}, ` +
        `lr=${args.learningRate}, batch=${args.batchSize}, width_floor_pts=${args.widthFloorPts}, ` +
        `width_floor_weight=${args.widthFloorWeight}, baseline_dropout=${args.baselineDropout}, ` +
        `baseline_noise_std=${args.baselineNoiseStd}`);
    const setLearningRate = (lr) => {
        const opt = optimizer;
        if (opt.setLearningRate)
            opt.setLearningRate(lr);
        else if (opt.learningRate != null)
            opt.learningRate = lr;
    };
    const computeMaeFromPreds = (preds, ys) => tf.tidy(() => {
    ...
    const clipGradients = (grads, clipNorm) => tf.tidy(() => {
    ...
    const guardrailCheck = (samples, rate) => {
        ...
        if (rate > 0.01 && actualRate === 0)
            console.warn("Guardrail: identityDropoutRate > 0 but no corps dropped!");
        return actualRate;
    };
    ...
    for (let epoch = args.startEpoch; epoch < args.startEpoch + args.epochs; epoch++) {
        ...
        if (args.noJudgeBias)
            scales.judgeBias = 0;
        if (args.noCorpsResidual)
            scales.corps = 0;
        const seqLen = provider.getSequenceLength();
        const epochSamples = buildSamples(provider.sampleRows(args.samplesPerEpoch, args.seed + epoch), stats, seqLen, weights.identityDropoutRate, args.seed + epoch, epoch, args.baselineDropout, args.baselineNoiseStd);
        const dropRate = guardrailCheck(epochSamples, weights.identityDropoutRate);
        console.log(`\nEpoch ${epoch}: Weights ${JSON.stringify(weights)}, Scales ${JSON.stringify(scales)}, SeqLen ${seqLen}, ID_Drop ${dropRate.toFixed(3)} `);
        ...
        for (const batch of batchGenerator(epochSamples, args.batchSize, true, args.seed + epoch, scales)) {
            ...
            const { value, grads } = tf.variableGrads(() => tf.tidy(() => {
                ...
                return lossFn(ys, preds, weights, xs.history_len, xs.show_id, false, regLoss);
            }));
            ...
            optimizer.applyGradients(clippedMap);
        ...
        if (cachedValSamples.length) {
            ...
            for (const batch of batchGenerator(cachedValSamples, args.batchSize, false, args.seed, scales)) {
                ...
                const lossTensor = lossFn(ys, preds, weights, xs.history_len, xs.show_id);
            ...
            for (let i = 0; i < 5; i++) {
                ...
                if (c > 0) {
                    console.log(`${bLabels[i].padEnd(4)} | ${c.toString().padEnd(5)} | ` +
        ...
        if (args.swa && epoch >= swaStartEpoch && (epoch - swaStartEpoch) % swaInterval === 0) {
            ...
            if (!swaWeights) {
                ...
                const updated = swaWeights.map((avg, idx) => {
        ...
        console.log(`Epoch ${epoch}: loss = ${trainLoss.toFixed(6)} ` +
