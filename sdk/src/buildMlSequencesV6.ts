...
export const ensureSequenceTablesV6 = Effect.gen(function* () {
  ...
  yield* (sql`
    CREATE TABLE IF NOT EXISTS ml_sequence_rows_v6 (
...
export const buildSequencesV6 = Effect.gen(function* () {
  ...
  for (const season of SEASONS) {
    ...
    // CRITICAL: Using V6 query which targets caption_scores
...
const insertBatch = (sql: SqlClient.SqlClient, rows: any[]) =>
  Effect.forEach(
    ...
    (row) =>
      sql`
        INSERT OR REPLACE INTO ml_sequence_rows_v6 (
          season,
          competition_slug,
          competition_date,
          division_name,
...
Effect.runPromise(buildSequencesV6.pipe(Effect.provide(SqlLayer)))
  .then(() => console.log("Done building V6 sequences."))
  .catch((error) => {
    console.error(error);
    process.exitCode = 1;
  });
