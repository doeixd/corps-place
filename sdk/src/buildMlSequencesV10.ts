...
const TIMESTEP_FEATURES = 7 + 1 + 11 + CAPTION_COUNT * CAPTION_FEATURES + OPPONENT_TIMESTEP_FEATURES + 4 + COMPARATIVE_FEATURES + 1 + 2; // +1 shows_count, +2 cyclic date
const STATIC_FEATURES = 53 + 12 + 8 + 3 + 4 + 27 + 16 + 8 + 1 + 2 + 2; // +2 target month/day, +2 premiere month/day
...
export const buildSequencesV10 = (seasons: string[] = SEASONS) => Effect.gen(function* () {
  const sql = yield* (SqlClient.SqlClient);

  yield* (sql`
    CREATE TABLE IF NOT EXISTS ml_sequence_rows_v10 (
      row_id INTEGER PRIMARY KEY AUTOINCREMENT,
      season TEXT NOT NULL,
      competition_slug TEXT NOT NULL,
      competition_date TEXT NOT NULL,
      ...
      corps_key TEXT NOT NULL,
      corps_id INTEGER,
      caption_id TEXT NOT NULL, -- The specific caption for this flattened row
      judge_id TEXT NOT NULL,    -- The specific judge for this flattened row
      judge_index INTEGER,       -- Mapping to JudgeIndexMap
      x_sequence_json TEXT NOT NULL,
      x_static_json TEXT NOT NULL,
      y_judge_score REAL NOT NULL,
      y_judge_content REAL,
      y_judge_achievement REAL,
      y_total REAL NOT NULL,     -- Overall corps total (for consistency loss)
      agnostic_show_id INTEGER,  -- Agnostic show mapping
      split TEXT NOT NULL CHECK(split IN ('train','val','test')),
      created_at TEXT NOT NULL
    )
  `);

  yield* (sql`CREATE INDEX IF NOT EXISTS idx_ml_v10_corps_show ON ml_sequence_rows_v10(corps_key, competition_slug);`);
  ...
  for (const season of seasons) {
    ...
    for (const division of DIVISIONS) {
      ...
      for (const [corpsKey, shows] of corpsMap.entries()) {
        ...
        const prevRank = prevSeasonRanks[season]?.[division]?.[corpsKey] ?? defaultRank;
        const premiereDate = new Date(shows[0].date);
        const premiereMonthNorm = premiereDate.getMonth() / 11;
        const premiereDayNorm = (premiereDate.getDate() - 1) / 30;
        ...
        for (let i = 0; i < shows.length; i++) {
          ...
          for (let j = 0; j < SEQ_LEN; j++) {
            ...
            const gapToWinnerPrev = prevShow ? (competitionMap.get(prevShow.slug)?.leader_score ?? prevShow.total_score) - prevShow.total_score : 0;
            ...
            feats.push(normalizeGap(gapToWinnerPrev));
            ...
            // New features
            const showsCountSoFarNorm = Math.min(showIdx + 1, 40) / 40;
            const dayOfYear = Math.floor((showDate.getTime() - new Date(showDate.getFullYear(), 0, 0).getTime()) / 86400000);
            const sinDate = Math.sin((2 * Math.PI * dayOfYear) / 365);
            const cosDate = Math.cos((2 * Math.PI * dayOfYear) / 365);
            feats.push(showsCountSoFarNorm, sinDate, cosDate);
          ...
          for (const assignment of judgeAssignments) {
            const eloKey = `${assignment.judge_id}:${season}:${assignment.caption_name}`;
            const elo = judgeEloCache.get(eloKey) ?? 1500;
            judgeElos.push(elo);
            if (!captionJudgeEloMap.has(assignment.caption_name)) captionJudgeEloMap.set(assignment.caption_name, []);
            captionJudgeEloMap.get(assignment.caption_name)!.push(elo);
          }
          ...
          let panelEloMean = 1500, panelEloStd = 0, panelEloMax = 1500, panelEloMin = 1500;
          ...
          const seasonCorpsElo = corpsEloMap.get(season)?.get(corpsKey) ?? new Map<string, number>();
          for (const caption of CAPTIONS) {
            const fullCaptionName = Object.keys(CAPTION_MAP).find(k => CAPTION_MAP[k] === caption) ?? caption;
          ...
          const divisionNameStr = targetShow.division_name?.toLowerCase() ?? "";
          const isWorldClass = divisionNameStr.includes("world") ? 1 : 0;
          const isOpenClass = divisionNameStr.includes("open") ? 1 : 0;
          const isAllAgeClass = divisionNameStr.includes("all-age") || divisionNameStr.includes("all age") ? 1 : 0;
          ...
          const x_static: number[] = [
            ...
            isAllAgeClass,
            // New static features
            new Date(targetShow.date).getMonth() / 11,
            (new Date(targetShow.date).getDate() - 1) / 30,
            premiereMonthNorm,
            premiereDayNorm,
          ];
          ...
          const split = (() => {
            ...
            if (season === "2022") return "val";
            return "train";
          })();

          // Load subcaption details for Content/Achievement split
          const subcaptionResults = yield* (sql<{
            caption_name: string;
            judge_id: string;
            subcaption_name: string;
            score: number;
          }>`SELECT caption_name, judge_id, subcaption_name, score FROM subcaption_scores WHERE competition_slug = ${targetShow.slug} AND corps_key = ${corpsKey}`);

          const judgeScoresBatch = yield* (sql<{
            caption_name: string;
            judge_id: string;
            score: number;
          }>`SELECT caption_name, judge_id, score FROM judge_scores WHERE competition_slug = ${targetShow.slug} AND corps_key = ${corpsKey}`);

          for (const js of judgeScoresBatch) {
            const capKey = CAPTION_MAP[js.caption_name];
            if (!capKey) continue;

            const subs = subcaptionResults.filter(s => s.caption_name === js.caption_name && s.judge_id === js.judge_id);
            let contentScore = 0;
            let achievementScore = 0;
            for (const sub of subs) {
              const cat = normalizeSubcaptionCategory(sub.subcaption_name);
              if (cat === "Content") contentScore += sub.score;
              else if (cat === "Achievement") achievementScore += sub.score;
            }
            ...
            allInserts.push({
              ...
              judge_index: JUDGE_INDEX_MAP[js.judge_id] ?? 0,
              x_sequence_json: JSON.stringify(x_sequence),
              x_static_json: JSON.stringify(x_static),
              y_judge_score: js.score,
              y_judge_content: contentScore > 0 ? contentScore : null,
              y_judge_achievement: achievementScore > 0 ? achievementScore : null,
              y_total: targetShow.total_score,
              agnostic_show_id: SHOW_INDEX_MAP[targetShow.slug.replace(/^\d{4}-/, "")] ?? 0,
              split,
      ...
      console.log(`Inserting ${allInserts.length} flattened judge rows for ${season} ${division}...`);
      const CHUNK_SIZE = 100;
      for (let i = 0; i < allInserts.length; i += CHUNK_SIZE) {
        const chunk = allInserts.slice(i, i + CHUNK_SIZE);
        yield* (insertBatchV10(sql, chunk));
      }
    }
  }
});

const insertBatchV10 = (sql: SqlClient.SqlClient, rows: any[]) =>
  Effect.forEach(
    rows,
    (row) =>
      sql`
        INSERT INTO ml_sequence_rows_v10 (
          season,
          competition_slug,
          competition_date,
          division_name,
...
Effect.runPromise(buildSequencesV10().pipe(Effect.provide(SqlLayer)))
  .then(() => console.log("Done building V10 sequences."))
  .catch((error) => {
    console.error(error);
    process.exitCode = 1;
  });
