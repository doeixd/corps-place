...
// Category rules live in merchCategories.ts (dependency-free — the app
// imports them for legacy category-URL redirects). Re-exported for existing
// sdk callers.
import {
  CATEGORY_SYNONYMS,
  normalizeCategory,
  bucketCategory,
  resolveCategory,
} from "./merchCategories.js";
export { CATEGORY_SYNONYMS, normalizeCategory, bucketCategory, resolveCategory };
      ...
      for (let page = 1; page <= maxPages; page++) {
        ...
        for (const raw of products) {
          ...
          out.push({
            ...
            category: resolveCategory(p.product_type ?? p.tags[0] ?? null, p.title),
          });
        }
      ...
      for (let page = 1; page <= maxPages; page++) {
        ...
        for (const raw of products) {
          ...
          out.push({
            ...
            variants: [],
            cartCapability: "prefill",
            category: resolveCategory(p.categories[0]?.name ?? null, p.name),
