...
const COMPARATIVE_FEATURES = 10; // NEW for V7: relative_total + relative_caption×8 + show_competitiveness
const TIMESTEP_FEATURES = 7 + 11 + CAPTION_COUNT * CAPTION_FEATURES + OPPONENT_TIMESTEP_FEATURES + 4 + COMPARATIVE_FEATURES + 3; // 98 + 3 = 101
const STATIC_FEATURES = 53 + 12 + 8 + 3 + 4 + 27 + 16 + 8 + 5; // 53 (V6) + ... + 5 new static = 136
...
export const buildSequencesV7 = (seasons: string[] = SEASONS) => Effect.gen(function* () {
  const sql = yield* (SqlClient.SqlClient);
  // V7 table already created in relational.ts schema
  ...
  // V7: Load show aggregates for comparative features
  ...
  // V7: Load judge Elo ratings (per season)
  console.log("Loading judge Elo ratings...");
  const judgeEloMap = new Map<string, Map<string, number>>(); // season -> caption -> avg_elo
  for (const season of seasons) {
  ...
  // V7: Load corps Elo ratings (per season/corps/caption)
  console.log("Loading corps Elo ratings...");
  const corpsEloMap = new Map<string, Map<string, Map<string, number>>>(); // season -> corps_key -> caption -> elo
  for (const season of seasons) {
  ...
  // V7: Load ALL judge Elo ratings into a global cache to avoid N+1 queries in the loop
  ...
  for (const season of seasons) {
    ...
    for (const division of DIVISIONS) {
      ...
      for (const row of rows) {
        ...
        // SKIP INVALID SCORES to prevent history poisoning
        // If the current row has a zero score, we don't want to start a new show entry for it
        // and we don't want to add it to an existing one if it's the driver.
        // However, the row-level check is safer.
        if (row.total_score <= 0) continue;
        ...
        for (let i = 0; i < shows.length; i++) {
          ...
          for (let j = 0; j < SEQ_LEN; j++) {
            ...
            if (showAgg) {
              ...
              // Show competitiveness (std of total scores)
              feats.push(showAgg.std_total / 10); // Normalize by dividing by 10
            } else {
              // No show aggregate data - push zeros
          ...
          // Build map of caption -> judge Elo
          ...
          for (const assignment of judgeAssignments) {
            ...
            // Look up judge Elo rating from cache
            ...
            // Map judge ID to index for embedding - Map to specific caption slot
          ...
          // Compute per-caption average judge Elo (8 values)
          for (const caption of CAPTIONS) {
            ...
            perCaptionJudgeElo.push((avgElo - 1500) / 200); // Normalize around 1500 ± 200
          }

          // Compute panel-level judge Elo stats (4 values)
          ...
          // V7: Get corps Elo features (per-caption)
          ...
          for (const caption of CAPTIONS) {
            ...
            perCaptionCorpsElo.push((corpsElo - 1500) / 200); // Normalize around 1500 ± 200
          }
          ...
          const x_static: number[] = [
            ...
            // V7: Judge Elo features (12 total)
            ...perCaptionJudgeElo, // 8 values (per-caption avg judge Elo)
            (panelEloMean - 1500) / 200, // Normalize panel mean
            panelEloStd / 100, // Normalize panel std
            (panelEloMax - 1500) / 200, // Normalize panel max
            (panelEloMin - 1500) / 200, // Normalize panel min
            // V7: Corps Elo features (8 values)
            ...perCaptionCorpsElo,
            // Rank baseline features (8 values)
...
const insertBatch = (sql: SqlClient.SqlClient, rows: any[]) =>
  Effect.forEach(
    ...
    (row) =>
      sql`
        INSERT OR REPLACE INTO ml_sequence_rows_v7 (
          season,
          competition_slug,
          competition_date,
          division_name,
