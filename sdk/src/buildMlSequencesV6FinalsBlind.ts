...
export const ensureSequenceTablesV6 = Effect.gen(function* () {
  ...
  yield* (sql`
    CREATE TABLE IF NOT EXISTS ml_sequence_rows_v6_finals_blind (
...
const insertBatch = (sql: SqlClient.SqlClient, rows: any[]) =>
  Effect.forEach(
    ...
    (row) =>
      sql`
        INSERT OR REPLACE INTO ml_sequence_rows_v6_finals_blind (
          season,
          competition_slug,
          competition_date,
          division_name,
...
Effect.runPromise(buildSequencesV6.pipe(Effect.provide(SqlLayer)))
  .then(() => console.log("Done building V6 Finals Blind sequences."))
  .catch((error) => {
    console.error(error);
    process.exitCode = 1;
  });
