...
export const buildSequencesV6 = Effect.gen(function* () {
  const sql = yield* (SqlClient.SqlClient);
  yield* (ensureSequenceTablesV6);
  ...
  for (const season of SEASONS) {
    ...
    const rows = yield* (MlQueries.querySeasonCaptionsV6(season, DIVISION));
    ...
    for (const [corpsKey, shows] of corpsMap.entries()) {
      ...
      for (let i = 0; i < shows.length; i++) {
        ...
        let split = "train";
        if (season === "2024") {
          if (targetShow.slug === "2024-dci-world-championship-finals") {
            split = "test";
          } else {
            split = "train";
          }
        } else {
          // PRODUCTION: All historical data (including 2022/2023) goes to train
          split = "train";
        }
...
const insertBatch = (sql: SqlClient.SqlClient, rows: any[]) =>
  Effect.forEach(
    ...
    (row) =>
      sql`
        INSERT OR REPLACE INTO ml_sequence_rows_v6_production (
          season,
          competition_slug,
          competition_date,
          division_name,
...
Effect.runPromise(buildSequencesV6.pipe(Effect.provide(SqlLayer)))
  .then(() => console.log("Done building V6 Production sequences."))
  .catch((error) => {
    console.error(error);
    process.exitCode = 1;
  });
