...
async function main() {
  ...
  for (let epoch = args.startEpoch; epoch < args.startEpoch + args.epochs; epoch++) {
    ...
    const valSamplesForEpoch = buildSamples(valSubset, stats, 15, 0.0, args.seed + 999);
    ...
    const trainDs = createDataset(epochSamples, args.batchSize, true, args.seed + epoch);
    ...
    if (valSamplesForEpoch.length) {
      const valDs = createDataset(valSamplesForEpoch, args.batchSize, false, args.seed);
      let valLossSum = 0;
      let valMaeSum = 0;
      ...
      const residualMeanTensor = tf.tensor1d(stats.residualMean, "float32");
      const residualStdTensor = tf.tensor1d(stats.residualStd.map((v) => (v > 1e-6 ? v : 1)), "float32");
      const recapMeanTensor = tf.tensor1d(stats.recapMean, "float32");
      const recapStdTensor = tf.tensor1d(stats.recapStd.map((v) => (v > 1e-6 ? v : 1)), "float32");
      const totalMeanTensor = tf.scalar(stats.totalMean);
      const totalStdTensor = tf.scalar(stats.totalStd > 1e-6 ? stats.totalStd : 1);
      ...
      while (true) {
        ...
        const recapMaePointsTensor = tf.mean(tf.mul(recapStdTensor, tf.abs(tf.sub(predRecap, trueRecap))));
        ...
        const totalMaePointsTensor = tf.mean(tf.mul(totalStdTensor, tf.abs(tf.sub(predTotal, trueTotal))));
  ...
  if (testRows.length > 0) {
    ...
    const testDs = createDataset(testSamples, args.batchSize, false, 42);
    ...
    const testIterator = await testDs.iterator();
    while (true) {
      const item = await testIterator.next();
      if (item.done) break;
      const { xs, ys } = item.value as { xs: BatchedInputs; ys: tf.Tensor };
      const batchSize = ys.shape[0] ?? 0;
      ...
      const predRecap = preds.slice([0, RESIDUAL_DIM], [-1, RECAP_DIM]);
      const trueRecap = ys.slice([0, CAPTION_COUNT], [-1, RECAP_DIM]);
      const recapMae = tf.mean(tf.mul(recapStdTensor, tf.abs(tf.sub(predRecap, trueRecap)))).dataSync()[0] ?? 0;

      const predTotal = preds.slice([0, RESIDUAL_DIM + RECAP_DIM], [-1, TOTAL_DIM]);
      const trueTotal = ys.slice([0, CAPTION_COUNT + RECAP_DIM], [-1, TOTAL_DIM]);
      const totalMae = tf.mean(tf.mul(totalStdTensor, tf.abs(tf.sub(predTotal, trueTotal)))).dataSync()[0] ?? 0;
