/**
 * trainModelV9-improved: Enhanced Model for 0.2 MAE Target
 *
 * IMPROVEMENTS OVER V9-test:
 * 1. Increased Model Capacity: LSTM 64→32 upgraded to 128→64 (+173K params)
 * 2. Lower Learning Rate: 0.0005 -> 0.0003 (finer optimization)
 * 3. Extended Training: 500 -> 800 epochs (more convergence time)
 * 4. Earlier Identity Dropout Ramp: Starts at epoch 150 instead of 250
 * 5. Improved Attention: Dense layer width 128->256 for richer representations
 * 6. Judge Embedding Capacity: 16->24 dimensions (better judge-specific learning)
 *
 * TARGET: 0.18-0.25 MAE (vs V9-test's 0.43 MAE)
 * ESTIMATED TRAINING TIME: 5-7 hours
 */
...
const MODEL_DIR = "./models/v9_improved";
const NORM_PATH = "./results/v9-improved-target-norm.json";
...
const BATCH_SIZE = 32;
const EPOCHS = 800;
const EARLY_STOPPING_PATIENCE = 80;
...
class V9LossScheduler {
  getWeights(epoch: number) {
    ...
    // IMPROVED: Earlier identity dropout ramp for faster corps-specific learning
    const t = Math.min(1.0, (epoch - 120) / 680); // Extended to 800 epochs
    const idDrop = (epoch < 100) ? 1.0 : Math.max(0.05, 1.0 - 0.95 * ((epoch - 100) / 200));

    return {
      // V9 CHANGE: Increased totalWeight to force consistency
      totalWeight: 0.10,
      // V9 CHANGE: Retained recapWeight anchor to prevent inertia drift
      recapWeight: 0.05,
      deltaWeight: 1.0,
      categoryWeight: 0.05,
      // V9 CHANGE: Capped quantileWeight to reduce safety bloat (0.35 -> 0.25 max)
      quantileWeight: 0.2 + 0.05 * t,
      consistencyWeight: 0.0,
...
function predictQuadratic(history: number[], fallback: number): number {
  ...
  if (Math.abs(det) < 1e-10) return fallback; // Singular matrix, use fallback

  const detA = sy * (sx2 * sx4 - sx3 * sx3) - sxy * (sx * sx4 - sx2 * sx3) + sx2y * (sx * sx3 - sx2 * sx2);
  const detB = n * (sxy * sx4 - sx2y * sx3) - sx * (sy * sx4 - sx2y * sx2) + sx2 * (sy * sx3 - sxy * sx2);
  const detC = n * (sx2 * sx2y - sx3 * sxy) - sx * (sx * sx2y - sx2 * sxy) + sx2 * (sx * sxy - sx2 * sy);
...
function createLoss(stats: TargetStats, widthFloorPts: number, rankingWeight: number) {
  ...
  const lossFn = (yTrue: tf.Tensor, yPred: tf.Tensor, weights: { totalWeight: number, recapWeight: number, deltaWeight: number, categoryWeight: number, quantileWeight: number, consistencyWeight: number }, historyLen: tf.Tensor, showIds: tf.Tensor, scheduledWidthFloorWeight: number, returnComponents = false, regLoss?: tf.Tensor) =>
    ...
    tf.tidy(() => {
      ...
      // V9 CHANGE: Squeezed target width from 2.56 (99%) to 1.64 (90%)
      const baseWidth = tf.mul(deltaStdTensor, 1.64);
...
async function main() {
  ...
  const d1 = tf.layers
    .dense({
      units: 256, // IMPROVED: 128->256 for richer feature representations
      activation: "relu",
      kernelRegularizer: tf.regularizers.l2({ l2: args.l2Reg }),
    })
  ...
  const d2 = tf.layers
    .dense({
      units: 128, // IMPROVED: 64->128 for better feature extraction
      activation: "relu",
      kernelRegularizer: tf.regularizers.l2({ l2: args.l2Reg }),
    })
  ...
  for (let epoch = args.startEpoch; epoch < args.startEpoch + args.epochs; epoch++) {
    ...
    if (cachedValSamples.length) {
      ...
      for (const batch of batchGenerator(cachedValSamples, args.batchSize, false, args.seed, scales)) {
        ...
        valDeltaMaeSum += maePointsValue * batchSize;
