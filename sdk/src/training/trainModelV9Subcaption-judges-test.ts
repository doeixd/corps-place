...
function parseArgs() {
  ...
  return {
    ...
    judgeEmbeddingDim: Number(get("--judge-embedding-dim", "32")),
    judgePanelUnits: Number(get("--judge-panel-units", "256")),
    judgePanelDropout: Number(get("--judge-panel-dropout", "0.15")),
  };
}
...
const CORPS_COUNT = Math.max(Object.keys(CORPS_INDEX_MAP).length, 4096);
...
function createLoss(stats: TargetStats, widthFloorPts: number, rankingWeight: number) {
  ...
  const deltaWeightTensor = tf.tensor1d(stats.deltaWeights.map((value) => Math.sqrt(value)), "float32");
...
async function main() {
  ...
  const judgeEmbedding = tf.layers.embedding({
    ...
    outputDim: args.judgeEmbeddingDim,
  ...
  const judgePanelProjection = tf.layers.dense({
    units: args.judgePanelUnits,
    activation: "relu",
    kernelRegularizer: tf.regularizers.l2({ l2: 1e-4 }),
    name: "judge_panel_projection",
  }).apply(judgeFlat) as tf.SymbolicTensor;
  const judgePanel = tf.layers.dropout({
    rate: args.judgePanelDropout,
    name: "judge_panel_dropout",
  }).apply(judgePanelProjection) as tf.SymbolicTensor;
  ...
  const concat = tf.layers.concatenate().apply([contextFlat, staticInput, judgePanel, baselineInput, lastStepFlat, showFlat]) as tf.SymbolicTensor;
  ...
  const judgeBiasRaw = tf.layers.dense({ units: CAPTION_COUNT, name: "judge_bias_raw", kernelRegularizer: tf.regularizers.l2({ l2: 1e-4 }) }).apply(judgePanel) as tf.SymbolicTensor;
  ...
  const widthConcat = tf.layers.concatenate({ name: "width_concat" }).apply([skipConcat, judgePanel, historyLenInput]) as tf.SymbolicTensor;
  ...
  console.log(`Model Capacity: ${args.lstm1Units * 2}→${args.lstm2Units * 2} BiLSTM, Dense 512→256, Judge Emb ${args.judgeEmbeddingDim}, Judge Panel ${args.judgePanelUnits}, Corps Emb 20, Show Emb 12`);
