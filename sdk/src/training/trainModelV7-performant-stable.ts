...
class V7LossScheduler {
  getWeights(epoch: number) {
    ...
    // Phase 2 (52-148): Category alignment + tiny quantile (width stabilization)
    if (epoch < 148) {
      return { totalWeight: 0.3, recapWeight: 0.5, residualWeight: 0.5, quantileWeight: 0.05, consistencyWeight: 0.2, identityDropoutRate: 1.0 };
    }
    ...
    if (epoch < 152) {
      ...
      return {
        ...
        quantileWeight: 0.05 + 0.25 * t,     // 0.05 → 0.3
        consistencyWeight: 0.2,
        identityDropoutRate: 1.0     // Stay masked
      };
    }
    // Phase 3 (152-260): Full quantiles, identity ramp to floor
    const t = Math.min(1.0, (epoch - 152) / 108); // Ramp over 108 epochs (until 260)
    const idDropout = 1.0 - 0.4 * t; // 1.0 → 0.6 (Floor at 0.6)
    const qWeight = 0.3 + 0.7 * t;   // 0.3 → 1.0 (Higher penalty for later phases)

    return {
      totalWeight: 0.1,
      recapWeight: 0.2,
      residualWeight: 0.4,
      quantileWeight: qWeight,
      consistencyWeight: 0.2,
...
function buildSamples(rows: DataRow[], stats: TargetStats, seqLen: number, identityDropoutRate: number, seed: number): Sample[] {
  ...
  for (const row of rows) {
    ...
    samples.push({
      xs: [slicedSeq, row.stat, slicedMask, row.judgeIndices, corpsId, row.agnosticShowId],
...
function createDataset(samples: Sample[], batchSize: number, shuffle: boolean, seed: number) {
  let dataset = tf.data.array(samples).map((sample) => ({
    xs: {
      ...
      agnostic_show_id: tf.tensor([sample.xs[5]], [1], "int32"),
    },
...
function buildBatchTensors(batchSamples: Sample[]): { xs: BatchedInputs; ys: tf.Tensor } {
  ...
  const agnosticShowIdData = new Int32Array(batchSize);
  ...
  for (let i = 0; i < batchSize; i++) {
    ...
    // Agnostic Show ID
    agnosticShowIdData[i] = sample.xs[5];
...
async function main() {
  ...
  const result = await client.execute(`
    SELECT season, corps_key, corps_id, x_sequence_json, x_static_json, judge_indices_json, y_residuals_json, y_recap_json, division_name, split, agnostic_show_id
  ...
  const concat = tf.layers.concatenate().apply([contextFlat, staticInput, judgeFlat, corpsFlat, showFlat]) as tf.SymbolicTensor;
  ...
  // STABILITY: Initialize deltas with negative bias to start with small widths (~0.05 sigma)
  const q10DeltaRaw = tf.layers.dense({
    units: CAPTION_COUNT,
    name: "q10_delta_raw",
    biasInitializer: tf.initializers.constant({ value: -3 })
  }).apply(skipConcat) as tf.SymbolicTensor;
  const q90DeltaRaw = tf.layers.dense({
    units: CAPTION_COUNT,
    name: "q90_delta_raw",
    biasInitializer: tf.initializers.constant({ value: -3 })
  }).apply(skipConcat) as tf.SymbolicTensor;
  ...
  const model = tf.model({ inputs: [seqInput, staticInput, maskInput, judgeIdsInput, corpsIdInput, agnosticShowInput], outputs: output });
  ...
  for (let epoch = args.startEpoch; epoch < args.startEpoch + args.epochs; epoch++) {
    ...
    if (cachedValSamples.length) {
      ...
      if (weights.residualWeight === 0) {
        ...
        // We use a simplified score for Phase 1 to ensure we track the relevant metrics
        valScore = valRecapMae + valTotalMae;
      } else {
        // Phase 2+: Full score with ramped coverage/width penalties
        // Note: We include recap/total weights to respect the curriculum focus
