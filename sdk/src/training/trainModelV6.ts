...
function parseArgs() {
  ...
  return {
    ...
    // Logging
    logCsv: get("--log-csv", "./results/lstm-v6-training-log.csv"),
...
async function main() {
  ...
  const trainRows = buildDataRows(rows.filter((row) => row.split === "train"));
  const valRows = buildDataRows(rows.filter((row) => row.split === "val"));
  ...
  if (!trainSubset.length || !valSubset.length) {
    throw new Error("Missing train/val data for V6 model.");
  }
  ...
  const baselines = computeBaselineMae(valSamples, stats);
  ...
  console.log(
    `Baselines (val residual MAE): zero=${baselines.baselineZero.toFixed(4)}, ` +
  ...
  for (let epoch = 0; epoch < args.epochs; epoch++) {
    ...
    while (true) {
      ...
      const absDiff = tf.abs(tf.sub(predDenorm, trueDenorm));
      const absSumByCaption = tf.sum(absDiff, 0);
      const withinCountByCaption = tf.sum(withinFloat, 0);
      const widthSumByCaption = tf.sum(tf.sub(upper, lower), 0);
      const absSumValues = absSumByCaption.dataSync();
      const withinCountValues = withinCountByCaption.dataSync();
      const widthSumValues = widthSumByCaption.dataSync();
      for (let idx = 0; idx < CAPTION_COUNT; idx++) {
        captionMaePointsSum[idx] += absSumValues[idx] ?? 0;
        captionCoverageWithin[idx] += withinCountValues[idx] ?? 0;
        captionWidthSum[idx] += widthSumValues[idx] ?? 0;
        captionCount[idx] += batchSize;
      }
      ...
      absDiff.dispose();
      absSumByCaption.dispose();
      withinCountByCaption.dispose();
      widthSumByCaption.dispose();
    ...
    if (valSamples.length) {
      const sampleBatch = valSamples.slice(0, Math.min(args.batchSize, valSamples.length));
      const sampleXs: [tf.Tensor, tf.Tensor, tf.Tensor] = [
        tf.tensor3d(sampleBatch.map((sample) => sample.xs[0]), [sampleBatch.length, SEQ_LEN, FEAT_DIM]),
        tf.tensor2d(sampleBatch.map((sample) => sample.xs[1]), [sampleBatch.length, STATIC_DIM]),
        tf.tensor2d(sampleBatch.map((sample) => sample.xs[2]), [sampleBatch.length, SEQ_LEN]),
      ];
      const sampleYs = tf.tensor2d(sampleBatch.map((sample) => sample.ys), [sampleBatch.length, TARGET_DIM]);
      const samplePreds = model.predict(sampleXs) as tf.Tensor;
      const components = lossFn(sampleYs, samplePreds, true) as {
        total: tf.Tensor;
        residual: tf.Tensor;
        recap: tf.Tensor;
        totalHead: tf.Tensor;
        consistency: tf.Tensor;
        widthFloor: tf.Tensor;
      };
      console.log(
        `  components: res=${(components.residual.dataSync()[0] ?? 0).toFixed(4)} ` +
        `recap=${(components.recap.dataSync()[0] ?? 0).toFixed(4)} ` +
        `total=${(components.totalHead.dataSync()[0] ?? 0).toFixed(4)} ` +
        `cons=${(components.consistency.dataSync()[0] ?? 0).toFixed(4)} ` +
        `width_floor=${(components.widthFloor.dataSync()[0] ?? 0).toFixed(4)}`
      );
      tf.dispose([sampleXs[0], sampleXs[1], sampleXs[2], sampleYs, samplePreds]);
      components.total.dispose();
      components.residual.dispose();
      components.recap.dispose();
      components.totalHead.dispose();
      components.consistency.dispose();
      components.widthFloor.dispose();
    }
    ...
    const trainMae = trainCount ? trainMaeSum / trainCount : 0;
    const valLoss = valCount ? valLossSum / valCount : 0;
    const valMae = valCount ? valMaeSum / valCount : 0;
    const valMaePoints = valCount ? valMaePointsSum / valCount : 0;
    const coverage = coverageCount ? coverageWithin / coverageCount : 0;
    const intervalWidth = valCount ? intervalWidthSum / valCount : 0;
    const widthNorm = valCount ? widthNormSum / valCount : 0;
    const widthFloorPct = coverageCount ? widthFloorCount / coverageCount : 0;
    const underCoverage = Math.max(0, SCORE_COVERAGE_TARGET - coverage);
    const valScore = valMaePoints + SCORE_COVERAGE_WEIGHT * underCoverage;
    const captionMaePoints = CAPTIONS.map((caption, idx) =>
      captionCount[idx] ? captionMaePointsSum[idx] / captionCount[idx] : 0
    );
    const captionCoverage = CAPTIONS.map((caption, idx) =>
      captionCount[idx] ? captionCoverageWithin[idx] / captionCount[idx] : 0
    );
    const captionWidth = CAPTIONS.map((caption, idx) =>
      captionCount[idx] ? captionWidthSum[idx] / captionCount[idx] : 0
    );
    const elapsed = ((Date.now() - startTime) / 1000).toFixed(2);

    console.log(
      `Epoch ${epoch}: loss=${trainLoss.toFixed(6)} val_loss=${valLoss.toFixed(6)} ` +
      `val_mae=${valMae.toFixed(6)} val_mae_pts=${valMaePoints.toFixed(4)} ` +
      `cov_10_90=${coverage.toFixed(3)} width_10_90=${intervalWidth.toFixed(3)} ` +
      `width_norm=${widthNorm.toFixed(3)} width_floor_pct=${widthFloorPct.toFixed(3)} ` +
      `score=${valScore.toFixed(4)} ` +
      `base_zero=${baselines.baselineZero.toFixed(4)} base_mean=${baselines.baselineMean.toFixed(4)} ` +
      `base_ema=${baselines.baselineEma.toFixed(4)} time=${elapsed}s`
    );
    console.log(
      `  caption_mae_pts=${CAPTIONS.map((caption, idx) => `${caption}:${captionMaePoints[idx]!.toFixed(4)}`).join(" ")}`
    );
    console.log(
      `  caption_cov_10_90=${CAPTIONS.map((caption, idx) => `${caption}:${captionCoverage[idx]!.toFixed(3)}`).join(" ")}`
    );
    console.log(
      `  caption_width_10_90=${CAPTIONS.map((caption, idx) => `${caption}:${captionWidth[idx]!.toFixed(3)}`).join(" ")}`
    );

    if (csvPath) {
      const captionMaePointsCsv = captionMaePoints.map((value) => value.toFixed(4)).join(",");
      const captionCoverageCsv = captionCoverage.map((value) => value.toFixed(3)).join(",");
      const captionWidthCsv = captionWidth.map((value) => value.toFixed(3)).join(",");
      const csvLine =
        `${args.trialId ?? "default"},${epoch},${trainLoss.toFixed(6)},${valLoss.toFixed(6)},${trainMae.toFixed(6)},${valMae.toFixed(6)},` +
        `${lr.toFixed(6)},${elapsed},${args.lstm1Units},${args.lstm2Units},${args.dropoutLstm},${args.batchSize},` +
        `${args.widthFloorPts},${args.widthFloorWeight},${widthNorm.toFixed(3)},${widthFloorPct.toFixed(3)},` +
        `${captionMaePointsCsv},${captionCoverageCsv},${captionWidthCsv}\n`;
      if (!csvInitialized) {
        const captionMaeHeaders = CAPTIONS.map((caption) => `mae_pts_${caption}`).join(",");
        const captionCoverageHeaders = CAPTIONS.map((caption) => `cov_10_90_${caption}`).join(",");
        const captionWidthHeaders = CAPTIONS.map((caption) => `width_10_90_${caption}`).join(",");
        const header =
          "trial_id,epoch,train_loss,val_loss,train_mae,val_mae,learning_rate,elapsed_sec,lstm1_units,lstm2_units,dropout,batch_size,width_floor_pts,width_floor_weight,width_norm,width_floor_pct," +
          `${captionMaeHeaders},${captionCoverageHeaders},${captionWidthHeaders}\n`;
    ...
    const improved = valScore < bestScore - 1e-4;
    ...
    if (improved) {
      ...
      if (shouldSave) {
        const bestSaveDir = path.join(runDir, `best_${epoch}_${now}`);
        await saveModel(model, bestSaveDir);
        const meta = {
          epoch,
          bestScore,
          valMaePoints,
        ...
        fs.writeFileSync(path.join(bestSaveDir, "best-meta.json"), JSON.stringify(meta, null, 2));
        bestSavedEpoch = epoch;
        lastBestSaveMs = now;
        console.log(`Saved BEST checkpoint @ epoch ${epoch} score=${bestScore.toFixed(4)} -> ${bestSaveDir}`);
      }
    } else {
      patience += 1;
  ...
  if (valSamples.length) {
    const valSeqTensor = tf.tensor3d(
      valSamples.map((sample) => sample.xs[0]),
      [valSamples.length, SEQ_LEN, FEAT_DIM]
    );
    const valStaticTensor = tf.tensor2d(
      valSamples.map((sample) => sample.xs[1]),
      [valSamples.length, STATIC_DIM]
    );
    const valMaskTensor = tf.tensor2d(
      valSamples.map((sample) => sample.xs[2]),
      [valSamples.length, SEQ_LEN]
    );
    const valResidualsTensor = tf.tensor2d(
      valSamples.map((sample) => sample.ys.slice(0, CAPTION_COUNT)),
      [valSamples.length, CAPTION_COUNT]
    );
    const preds = model.predict([valSeqTensor, valStaticTensor, valMaskTensor]) as tf.Tensor;

    const predQ50 = preds.slice([0, CAPTION_COUNT], [-1, CAPTION_COUNT]);

    const residualMean = tf.tensor1d(stats.residualMean, "float32");
    const residualStd = tf.tensor1d(stats.residualStd.map((value) => (value > 1e-6 ? value : 1)), "float32");

    const predDenorm = tf.add(tf.mul(predQ50, residualStd), residualMean);
    const actualDenorm = tf.add(tf.mul(valResidualsTensor, residualStd), residualMean);
    const maePoints = tf.mean(tf.abs(tf.sub(predDenorm, actualDenorm)));
    console.log(`MAE (points): ${maePoints.dataSync()[0] ?? 0}`);
  ...
  disposeLoss();
  console.log(`Saving model to ${runDir}...`);
