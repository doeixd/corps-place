/**
 * trainModelV9-test: The "Squeeze" (Calibrated V9)
 * 
 * CHANGES FROM V9:
 * 1. Reduced Width Target Multiplier: 2.56 -> 1.64 (Targeting 90% CI instead of 99%)
 * 2. Increased Total Weight: 0.01 -> 0.10 (Force arithmetic consistency)
 * 3. Retained Recap Anchor: Phase C recapWeight 0.0 -> 0.05 (Prevent inertia drift)
 * 4. Capped Quantile Weight: 0.35 -> 0.25 (Reduce incentive for massive safety nets)
 */
...
const MODEL_DIR = "./models/v9_curriculum"; // Changed directory
const NORM_PATH = "./results/v9-curriculum-target-norm.json";
...
class V9LossScheduler {
  getWeights(epoch: number) {
    ...
    // V9: Significantly different Phase C config
    const t = Math.min(1.0, (epoch - 120) / 280);
    const idDrop = (epoch < 150) ? 1.0 : Math.max(0.1, 1.0 - 0.9 * ((epoch - 150) / 150));
