...
const main = async () => {
  ...
  for (const ev of events) {
    ...
    // direct event-page subs (target_slug = the event slug itself — the notify
    // button on /scores/$slug and the event prediction page), AND the '*'
    // wildcard (admins subscribed to every show's scores).
