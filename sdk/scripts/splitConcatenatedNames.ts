...
const main = async () => {
  ...
  let repointed = 0, cloned = 0, removed = 0;
  for (const s of staff) {
    ...
    for (let i = 2; i <= parts.length - 2; i++) {
      ...
      if (personIds.has(a) && personIds.has(b) && a !== s.person_id && b !== s.person_id && !hasRoleWord(a) && !hasRoleWord(b)) { split = [a, b]; break; }
    }
    if (!split) continue;
    const [a, b] = split;
    ...
    for (const row of asg) {
      ...
      const targets = aHas && !bHas ? [a] : bHas && !aHas ? [b] : [a, b];
  ...
  console.log(`\n${DRY ? '(dry-run)' : 'APPLIED'} — re-pointed ${repointed} rows (+${cloned} clones), removed ${removed} bogus staff`);
  process.exit(0);
};
main();
