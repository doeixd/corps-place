...
async function main() {
  ...
  await client.execute({
    sql: `
      CREATE TABLE IF NOT EXISTS corps_historical_features_v5 (
  ...
  for (const history of corpsHistory.values()) {
    ...
    const bestSeason = Math.min(...history.seasons.filter((entry) => entry.rank === bestRank).map((entry) => entry.season));
  ...
  await client.execute({
    sql: `DELETE FROM corps_historical_features_v5`,
  });

  for (const row of insertRows) {
    await client.execute({
      sql: `
        INSERT INTO corps_historical_features_v5 (
          corps_key,
