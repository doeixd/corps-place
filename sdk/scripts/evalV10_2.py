#!/usr/bin/env python3
"""3-way paired evaluation on the honest held-out 2026 slice (2026-07-12 .. 07-16).

Compares the agnostic control ensembles of:
  V10   (2013-2025, no 2026)         -- prefix "V10"   seeds 42-49
  v10.1 (+2026 thru 07-06)           -- prefix "v10_1" seeds 42/43/44
  v10.2 (+2026 thru 07-11)           -- prefix "v10_2" seeds 42-49
on the IDENTICAL 07-12..07-16 holdout. Each model's replay JSON was produced with
its OWN saved target-norm (--norm-path). Ensemble = average the 8 caption p50s across
seeds, then derive total. Reports per-seed + ensemble recap/total MAE, SIGNED total
bias, by history bucket + division, and show-clustered bootstrap CIs including on the
v10.2-V10 and v10.2-v10.1 deltas.
"""
import json, csv, statistics, glob, sys, os

COHORT_CSV = "results/v10_2_eval/holdout_cohort.csv"
EVAL_DIR = "results/v10_2_eval"
BOOT = 2000
SEED = 12345
...
if __name__ == "__main__":
    ...
    print(f"Held-out 2026 slice: {len(cohort)} rows / {nshows} shows (2026-07-12 .. 07-16). "
          f"Control lane, identity-agnostic. Each model uses its own saved target-norm.")
    ...
    if v10 and v12:
        ens10, A10, k10 = v10
        ens12, A12, k12 = v12
        keys = sorted(set(k10) & set(k12))
        print("\n=== 3-WAY PAIRED (identical rows) ===")
        # headline recap/total ensemble table
        rows = [("V10", ens10, A10), ]
        if v11:
            ens11, A11, k11 = v11
        ...
        pair("v10.2 vs V10", (ens10, A10), (ens12, A12), keys)
        if v11:
            pair("v10.2 vs v10.1", (ens11, A11), (ens12, A12), keys)
            pair("v10.1 vs V10", (ens10, A10), (ens11, A11), keys)
