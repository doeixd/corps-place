// Static (no-motion, no-floating-ui) twin of FullRecapTable, rendered on the
// read-only /scores index + show pages. Identical output, but plain <tr>/<button>
// instead of motion.tr + base-ui Tooltip, and the -static child components — so
// the /scores bundle sheds motion/react and the floating-ui positioning stack
// (measured: ~20-chunk waterfall on nav). The animated FullRecapTable stays for
// the interactive prediction pages.
...
import { RecapSectionRowStatic } from '@/components/prediction/recap-section-row-static';
import { RecapHeadCellsStatic } from '@/components/prediction/recap-head-cells-static';
 ...
 */
export function FullRecapTableStatic({
  recap,
  corpsLookup,
  classFilters,
  onSetClassFilters,
  ...
  yearSlug,
  onStickyScroll,
}: // animateRows is accepted (callers share the animated table's API) but ignored —
// this variant never animates.
FullRecapTableProps) {
...
function SortButton({
  ...
  return (
    <span className="inline-flex h-5 items-center justify-center gap-1 whitespace-nowrap">
      {/* Native title (no base-ui Tooltip) + CSS-rotated arrow (no motion). */}
      <button
        type="button"
        onClick={onSort}
        title={leafTooltip(leaf)}
      ...
      >
        ...
        <span
          className={cn(
            'inline-flex transition-[filter,transform] duration-200 ease-out',
            dir === 'asc' && 'rotate-180',
