 ...
 * editor-agnostic FreeFormDoc envelope on every change. When `citations` are
 * supplied, a toolbar lets the author insert an inline citation (M11b); the
 * CitationNode is always registered so a stored doc containing one loads safely.
 */
export function LexicalFreeForm({
  ...
  citations?: readonly EditorCitation[];
}) {
...
function CitationToolbar({ citations }: { citations: readonly EditorCitation[] }) {
