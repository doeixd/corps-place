 ...
 *
 * Every merged seedable row carries `source` + `sourceAuthority` (set by the
 * read-model builders) and a per-row `scrapeDiverged` flag (set by the nightly
 * reconciler). These small badges surface that provenance so a reader can tell a
 * yearbook-sourced fact from a generic scrape, and so a divergence banner names
 * the authority it diverged from ("differs from the official 2017 DCI Yearbook").
 */
