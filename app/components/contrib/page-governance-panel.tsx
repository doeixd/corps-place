...
/**
 * Moderator governance + stewardship sidebar (M9). Page lock level (open / trusted /
 * mod) gates the edit capability (§6.4); stewards are page-watchers any signed-in
 * editor can join. CLIENT component — server-fns are value-imported (createServerFn
 * stripped on the client); the `ShowGovernance`/`ShowPageLock` types are import-only.
 */
  ...
  return (
    <Card>
      <CardContent className="space-y-4 py-5">
        ...
        {governance.stewards.length > 0 ? (
          <div className="space-y-1 rounded-md bg-foreground/5 px-2.5 py-2">
            {governance.stewards.map((steward) => (
              <p key={steward.userId} className="truncate text-xs text-text-secondary">
                {steward.name || 'A contributor'}
              </p>
            ))}
            {governance.stewardCount > governance.stewards.length ? (
