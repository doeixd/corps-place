...
import { loadServerGeometry } from '@/lib/tour/static-map.server';
...
async function seasonMapDataUri(
  season: string
): Promise<{ uri: string; corps: number; shows: number } | null> {
  ...
  // Topology loading + path building shared with /api/tour-map (static-map.server).
  const [geo, sharp] = await Promise.all([loadServerGeometry(), import('sharp')]);
  const states = geo.statesPath;
  const nation = geo.nationPath;

  const routes = competitive
    .map((c) => {
      const pts = c.stops
        .map(([, , lat, lng]) => geo.project(lng, lat))
