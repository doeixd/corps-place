...
import { getCorpsMerch } from '@/lib/server-fns/hybrid';
...
export const ServerRoute = createServerFileRoute('/read-model/corps-merch/$slug').methods({
  GET: async ({ params }) => {
    const id = params.slug.replace(/\.json$/i, '');
    return new Response(JSON.stringify(await getCorpsMerch({ data: id })), { headers: SHARD_HEADERS });
  },
});
