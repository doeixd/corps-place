...
export const ServerRoute = createServerFileRoute('/read-model/staff/$personId').methods({
  GET: async ({ params }) => {
    const id = params.personId.replace(/\.json$/i, '');
    return new Response(JSON.stringify(await getStaffProfile({ data: id })), { headers: SHARD_HEADERS });
  },
});
